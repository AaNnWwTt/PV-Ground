...
from .pv_backbone import pv_backbone
from .modules import (
...
class PVGround(nn.Module):
    ...
    def __init__(self, model_cfg, num_class=256, num_obj_class=485,
                 input_feature_dim=3,
                 num_queries=256,
...
#-------------------text-head decoder-------------------------------------
        ...
        self.backbone_net = pv_backbone(model_cfg)
    ...
    def forward(self, inputs):
        ...
        points_xyz = end_points['seed_xyz'].contiguous()
        points_features = end_points['seed_features']
        text_feats = end_points['text_feats']
        text_padding_mask = end_points['text_attention_mask']
        end_points['coords'] = inputs['points'][..., 1:4].view(inputs['batch_size'], -1, 3).contiguous()
        ...
        points_features, text_feats = self.cross_encoder(
            vis_feats=points_features.contiguous(),
        ...
        source_xzy = inputs['points'][..., 1:4].view(inputs['batch_size'], -1, 3).contiguous()  # [B, 50000, 3]
        super_features = []
        super_xyz_list = []
        for bs in range(source_xzy.shape[0]):
            super_xyz = scatter_mean(source_xzy[bs], superpoint[bs], dim=0).unsqueeze(0)
...
#---------------------------text decoder-----------------------------------
        ...
        for bs in range(query.shape[0]):
            ...
            prediction_masks.append(pred_masks.expand(1,256,pred_masks.shape[-1]))
...
class GumbelSampling(nn.Module):
    def __init__(self, in_features, sampling_n, dropout=0.1, activation_fn=None):
    ...
    def forward(self, x, tau=1., hard=False, mask=None):
        """
        x.shape = [bz, point_num, feature_len]
        """
        ...
        sample_weights = self.sampler(x) # [bz, N, sampling_n]
        ...
        return attn  # [batch_size, sampling_n, num_points]