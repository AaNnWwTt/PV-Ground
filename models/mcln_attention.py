...
def multi_head_attention_forward(
    ...
    #
    attn_output, attn_output_weights,src_weight = _scaled_dot_product_attention(q, k, v, attn_mask, dropout_p)
    ...
    if need_weights:
        ...
        src_weight=src_weight.view(bsz, num_heads, tgt_len, src_len)
        
        if average_attn_weights:
            attn_output_weights = attn_output_weights.sum(dim=1) / num_heads
            src_weight=src_weight.sum(dim=1) / num_heads
            
        if not is_batched:
            # squeeze the output if input was unbatched
            attn_output = attn_output.squeeze(1)
            attn_output_weights = attn_output_weights.squeeze(0)
            src_weight=src_weight.squeeze(0)
        return attn_output, attn_output_weights,src_weight
    else:
        if not is_batched:
            # squeeze the output if input was unbatched
            attn_output = attn_output.squeeze(1)
...
class MultiheadAttention(Module):
    ...
    Outputs:
        ...
        if not self._qkv_same_embed_dim:
            ...
            attn_output, attn_output_weights,src_weights = multi_head_attention_forward(
                query, key, value, self.embed_dim, self.num_heads,
                self.in_proj_weight, self.in_proj_bias,
                self.bias_k, self.bias_v, self.add_zero_attn,
                self.dropout, self.out_proj.weight, self.out_proj.bias,
        ...
        if self.batch_first and is_batched:
            ...
            return attn_output, attn_output_weights,src_weights 



if __name__=="__main__":
    # Create a MultiheadAttention module
    embed_dim, num_heads = 10, 5
    multihead_attn = MultiheadAttention(embed_dim, num_heads)
    print(multihead_attn)

    # Create input tensors
    query = torch.randn(20, 4, embed_dim)
    key = value = torch.randn(25, 4, embed_dim)

    # Call the module
    attn_output, attn_output_weights,_ = multihead_attn(query, key, value)
