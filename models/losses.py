...
# (可以加在 generalized_box_iou3d 函数附近)
def calculate_diou_3d(boxes1, boxes2):
    """
    Computes 3D DIoU.
    boxes1, boxes2: [N, 6] and [M, 6] in xyzxyz format.
    Returns a [N, M] pairwise matrix of DIoU values.
    """
    assert (boxes1[:, 3:] >= boxes1[:, :3]).all()
    assert (boxes2[:, 3:] >= boxes2[:, :3]).all()
    
    # 1. Get IoU (使用您已有的 _iou3d_par)
    iou, _ = _iou3d_par(boxes1, boxes2) # [N, M]

    # 2. Find the smallest enclosing box
    lt = torch.min(boxes1[:, None, :3], boxes2[:, :3]) # [N, M, 3]
    rb = torch.max(boxes1[:, None, 3:], boxes2[:, 3:]) # [N, M, 3]
    wh = (rb - lt).clamp(min=0) # [N, M, 3]
    
    # 3. Calculate squared diagonal of enclosing box (c^2)
    c2 = wh[:, :, 0]**2 + wh[:, :, 1]**2 + wh[:, :, 2]**2 + 1e-6 # [N, M]

    # 4. Calculate squared distance between centers (rho^2)
    center1 = (boxes1[:, :3] + boxes1[:, 3:]) / 2 # [N, 3]
    center2 = (boxes2[:, :3] + boxes2[:, 3:]) / 2 # [M, 3]
    
    rho2 = (center1[:, None, 0] - center2[:, 0])**2 + \
           (center1[:, None, 1] - center2[:, 1])**2 + \
           (center1[:, None, 2] - center2[:, 2])**2 # [N, M]

    # 5. Calculate DIoU = IoU - (rho^2 / c^2)
    diou = iou - (rho2 / c2)
    return diou
...
class SetCriterion(nn.Module):
    ...
    def loss_boxes(self, outputs, targets, indices, num_boxes, auxi_indices):
        ...
        # # +++ 添加新的 DIoU Loss +++
        # # 1. 计算 DIoU (使用您在步骤1中添加的函数)
        # diou_values = torch.diag(calculate_diou_3d(
        #     box_cxcyczwhd_to_xyzxyz(src_boxes),
        #     box_cxcyczwhd_to_xyzxyz(target_boxes)
        # ))
        
        # # 2. DIoU Loss = 1 - DIoU
        # loss_diou = 1 - diou_values
        
        # losses = {}
        # losses['loss_diou'] = loss_diou.sum() / num_boxes # <-- 添加新的 loss
    ...
    def loss_masks(self, outputs, targets, indices, num_boxes, auxi_indices):
        ...
        if 'pred_masks' in outputs:
            for bs in range(len(outputs['pred_masks'])):
                ...
                weight = torch.ones_like(sp_src_masks,dtype=torch.float32)
                u1 = 0.5  
                sigma1 = 0.1  
                left = 1 / (np.sqrt(2 * math.pi) * np.sqrt(sigma1))
                right = np.exp(-(sp_src_masks.sigmoid().detach().cpu().numpy() - u1)**2 / (2 * sigma1))
                weight_numpy =  2 - left*right
...
def compute_hungarian_loss(end_points, num_decoder_layers, set_criterion,
    ...
    for prefix in prefixes:
        ...
        # loss_diou += losses.get('loss_diou', 0)
    ...
    loss = (
        ...
        +5*sp_loss_mask + sp_loss_dice + 10*corresponding_loss_mask + 2*corresponding_loss_dice + 10*adaptive_weight_loss_mask + 2*adaptive_weight_loss_dice
    )
    end_points['loss_ce'] = loss_ce
    end_points['loss_bbox'] = loss_bbox
    end_points['loss_giou'] = loss_giou
    # end_points['loss_diou'] = loss_diou
