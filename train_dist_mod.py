...
from models import PVGround
from src.joint_det_dataset import Joint3DDataset
from src.grounding_evaluator import GroundingEvaluator
from models import APCalculator, parse_predictions, parse_groundtruths

from pcdet.config import cfg_from_yaml_file
from pcdet.config import cfg as model_cfg
...
class TrainTester(BaseTrainTester):
    ...
    # logger.
    def __init__(self, args, model_cfg=None, training=True):
        """Initialize."""
        super().__init__(args)
        self.model_cfg = model_cfg
        pc_range = np.asarray(model_cfg.DATA_CONFIG.POINT_CLOUD_RANGE) 
        num_point_features=len(model_cfg.DATA_CONFIG.POINT_FEATURE_ENCODING.src_feature_list)
        self.data_processor = DataProcessor(model_cfg.DATA_PROCESSOR, pc_range, training, num_point_features)
    ...
    # BRIEF input data.
    def _get_inputs(self, batch_data):
        # return {
        ...
        voxel_batch = []
        for pc in batch_data['point_clouds']:
            batch_dict = {'points': pc.cpu().numpy(), 'use_lead_xyz': True}
            voxels = self.data_processor.forward(batch_dict)             
            # print(f'voxels_shape: {voxels["voxels"].shape}')

            voxel_batch.append(voxels)

        voxel_batch = self.data_processor.collate_batch(voxel_batch)                # ([B, N, 6]) xyz + colour
        for key, val in voxel_batch.items():
            # print(f'key: {key}')  # key: points, use_lead_xyz, voxels， voxel_coords， voxel_num_points, batch_size
            if not isinstance(val, np.ndarray):
                continue
            voxel_batch[key] = torch.from_numpy(val).float().to(batch_data['point_clouds'].device)

        # print(f'voxels_shape: {voxel_batch["voxels"].shape}')  # ([Nv, Mv, 6]) xyz + colour
        # print(f'points_shape: {voxel_batch["points"].shape}')  # ([Np, 7]) B + xyz + colour
        # print(f'voxels_coords_shape: {voxel_batch["voxel_coords"].shape}')
        return {
            # 'point_clouds': batch_data['point_clouds'].float(), # ([B, 50000, 6]) xyz + colour
            'voxels': voxel_batch['voxels'], # ([B, Nv, Mv, 6]) xyz + colour
            'points': voxel_batch['points'],  # ([Np, 7]) xyz + colour
            'voxel_coords': voxel_batch['voxel_coords'],  # ([B, Nv, 4]) bxyz
            'voxel_num_points': voxel_batch['voxel_num_points'],  # ([B, Nv]) number of points in each voxel
            'batch_size': voxel_batch['batch_size'],  # ([B]) batch size
            'text': batch_data['utterances'],                   # list[B]  text
            "det_boxes": batch_data['all_detected_boxes'],      # ([B, 132, 6]) groupfree detection boxes
            "det_bbox_label_mask": batch_data['all_detected_bbox_label_mask'],  # ([B, 132]) mask
            "det_class_ids": batch_data['all_detected_class_ids'],   # ([B, 132])  box id
...
if __name__ == '__main__':
    ...
    train_tester = TrainTester(opt, model_cfg)
